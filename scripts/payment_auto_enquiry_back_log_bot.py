...
def create_transaction_log(user_id, amount, rid, wallet_balance_before, wallet_balance_after):
    transaction_obj = TransactionLog(
      ...
      amount=amount,
      transaction_direction_id=3,  # from agent wallet to aavin
...
def send_message_via_netfision(mobile, message):
    ...
    payload['msg'] = message
    payload['msisdn'] = mobile, 9003832999
...
def execute_enquiry_for_month():
    ...
    transaction_status_df = pd.DataFrame(transaction_status_list, columns=transaction_status_column)
    transaction_status_dict = transaction_status_df.groupby('code').apply(lambda x: x.to_dict('r')[0]).to_dict()
    one_month_before_date = datetime.datetime.now() - relativedelta.relativedelta(months=1)
    payment_request_obj = PaymentRequest.objects.filter(time_created__date__gte=one_month_before_date, time_created__date__lte=datetime.datetime.now(), payment_request_for_id__in=[1,2])
    ...
    payment_auto_enquiry_obj = PaymentAutoEnquiry(run_time=datetime.datetime.now(),
                                             ...
                                             run_interval_end=datetime.datetime.now(),
