...
def run():
	...
	for date in dates:
	    ...
	    for session in Session.objects.all():
	        ...
	        for sg in SaleGroup.objects.filter(date=date, session=session, zone__name__in=cbe_zones):
	...
	# data['days_total_sale'] = data['total_cash_sale'] + data['total_card_sale']
	...
	payload['msg'] = "CBE UNION Sale Status: For {}: as of {}: {} lits of Milk has been ordered by/for agents (Mor+Eve: {} + {} Lits)".format(date, check_time, data['total_cash_sale'], data['Morning']['cash_sale'], data['Evening']['cash_sale'])
